/*!Updated: 01-07-2014, 12:34:11 PM */
...
(function() {
  this.getEventsFeed = function(programme, type, feed_id, count, keyword, width) {
    var getItemHTML, img_height, img_size, img_width, length, outputfeedHTML;
    if (programme == null) {
    ...
    getItemHTML = function(item) {
      ...
      return itemHTML += '<li> <div class="single-feed-container"> <div class="feed-image"> <div class="center-cropped"> <a href="' + item.event_url + '"> <img class="resrc" src="http://app.resrc.it/S=W' + img_size + '/C=W' + img_width + ',H' + img_height + '/' + events_image_url + '"> </a> </div> </div> <div class="title"> <a href="' + item.event_url + '" title="' + item.name + '">' + short_title + '<p class="date">' + ev_date + ', ' + ev_type + '</p> </a> </div> </div> </li>';
    };
    outputfeedHTML = function(data) {
      var output;
  ...
  $(document).ready(function() {
    if ($(".events-feed").length > 0) {
      return $.each($(".events-feed"), function() {
        var _this;
        _this = $(this);
        return getEventsFeed(_this.data('event-programme'), _this.data('event-type'), _this.data('feed-id'), _this.data('item-count'), _this.data('event-keyword'), _this.data('feed-width'));
      });
    }
  });
...
(function() {
  this.getNewsFeed = function(college, tag, category, feed_id, count, width) {
    var blog_url, feed_url, img_height, img_size, img_width, length;
    if (count === "") {
      count = 6;
    } else {
      count = parseInt(count, 10);
    ...
    return $.getJSON(feed_url, function(data) {
      ...
      $.each(data.posts, function(i, item) {
        ...
        if (i < count) {
          ...
          output += '<li> <div class="feed-image"> <div class="center-cropped"> <a href="' + item.url + '">';
          if (item.attachments.length) {
            output += '<img class="resrc" src="http://app.resrc.it/S=W' + img_size + '/C=W' + img_width + ',H' + img_height + '/' + item.attachments[0].url + '" alt="' + item.attachments[0].caption + '">';
          }
          return output += '</a> </div> </div> <div class="title"> <a href="' + item.url + '" title="' + item.title + '">' + short_title + '</a> </div> </li>';
        }
      });
  ...
  $(document).ready(function() {
    if ($(".news-feed").length > 0) {
      return $(".news-feed").each(function() {
        var _this;
        _this = $(this);
        return getNewsFeed(_this.data('news-college'), _this.data('news-tag'), _this.data('news-category'), _this.data('feed-id'), _this.data('item-count'), _this.data('feed-width'));
      });
    }
  });
